pro ex2_whitenoise_wavelet

repeat_n=1000
;repeat Monte Carlo test for repeat_n times (in the Torrence paper repeat_n=100000)
data_n=512 ;length of tiem series data
sample_arr=randomn(seed,data_n,repeat_n) ;generate white noise
alfa=0. ;test red-noise if (alfa NE 0)
...
power1=0
dj = (1./8.)*(2^power1)  ; this will produce 8 sub-octaves per octave, 
...
power2=ceil(log2(data_n*dt/s0))
j1 = power2/dj  ; this says do 9 powers-of-two with dj sub-octaves each
scale_n=j1+1    
power_arr=fltarr(scale_n,repeat_n) & pha_arr=power_arr

for i=0,repeat_n-1 do begin
  input_case=sample_arr[*,i]-mean(sample_arr[*,i]) ;make input with average 0.
  wave=wavelet(input_case,dt,mother='morlet',scale=scale,S0=s0,lag1=alfa,$
    period=period,PAD=pad,COI=coi,DJ=dj,J=j1)
  power=abs(wave)^2
...
normalized_var=mean(power_arr,dim=2)/(stddev(sample_arr)^2);normalize the result
plot,period,normalized_var,/xlog,xtitle='Period',ytitle='Normalized var',$
  title='Fig 5a, Normalized mean-variance',charsize=1.5
oplot,10^!X.CRANGE,[1,1],linestyle=2
